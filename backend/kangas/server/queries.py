...
def select_query(
    ...
    if group_by:
        ...
        select_sql = "SELECT {select_expr_as} FROM {databases} WHERE {where} GROUP BY column_0 ORDER BY {sort_by_field_name} {sort_desc} LIMIT {limit} OFFSET {offset}"
    ...
    if group_by:
        ...
        total_sql = "SELECT COUNT() FROM (SELECT {select_expr_as} FROM {databases} WHERE {where} GROUP BY column_0);"
        # Add asset messages:
        rows = list(rows)
        for r in range(len((rows))):
            row = dict(zip(select_columns, rows[r]))
